  ...
  return (
    <>
      ...
      <section className="hero-editorial">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-0">
          <div className="flex flex-col lg:flex-row lg:items-center lg:gap-16">
            <div className="flex-1 max-w-2xl pb-14 lg:pb-20">
              ...
              <h1
                className="anim-fade-up anim-delay-1 text-[42px] sm:text-[58px] lg:text-[66px] font-semibold leading-[1.04] tracking-[-0.03em] text-white mb-5"
        ...
        <div className="h-10 bg-gradient-to-b from-transparent to-[#f8f6f1]" />
      </section>
      ...
      <section className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="mb-8">
          ...
          <h2 className="text-[28px] sm:text-[34px] font-semibold tracking-tight text-[#111218]" style={{ fontFamily: "var(--font-display)" }}>
            Notre top 3 des hébergeurs web
          </h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {top3Picks.map((pick) => (
            <div key={pick.name} className={`group relative bg-white border rounded-2xl p-5 card-hover overflow-hidden ${pick.isBest ? "border-[#059669]/30 shadow-[0_0_0_1px_rgba(5,150,105,0.1),0_4px_20px_rgba(5,150,105,0.08)]" : "border-[#e3e0d8]"}`}>
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${pick.isBest ? "badge-best bg-[#059669] text-white" : "bg-[#f0ede6] text-[#6b7280]"}`} style={{ fontFamily: "var(--font-mono)" }}>{pick.badge}</span>
                </div>
              ...
              <h3 className="text-[18px] font-semibold text-[#111218] mb-2" style={{ fontFamily: "var(--font-display)" }}>{pick.name}</h3>
              <p className="text-[13px] text-[#6b7280] leading-relaxed mb-3 line-clamp-2">{pick.verdict}</p>
              <p className="text-[12px] text-[#aaa] mb-4" style={{ fontFamily: "var(--font-mono)" }}>{pick.price}</p>
              <Link href={pick.href} className="block text-center px-3 py-1.5 border border-[#e3e0d8] rounded-lg text-[12px] font-medium text-[#374151] hover:border-[#059669] hover:text-[#059669] transition-all">
                Voir l&apos;avis complet →
              </Link>
            </div>
          ))}
      ...
      {/* CATÉGORIES */}
      <section className="bg-[#f0ede6] border-y border-[#e3e0d8] py-16">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link href="/comparatifs" className="hidden sm:flex items-center gap-1 text-[13px] text-[#6b7280] hover:text-[#059669] transition-colors font-medium">Tout voir →</Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {categories.map((cat) => {
              const card = (
                <div className={`group bg-white border border-[#e3e0d8] rounded-2xl p-5 card-hover flex items-start gap-4 ${!cat.live ? "opacity-55 cursor-default" : "cursor-pointer"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      <h3 className={`text-[15px] font-semibold tracking-tight transition-colors ${cat.live ? "text-[#111218] group-hover:text-[#059669]" : "text-[#888]"}`} style={{ fontFamily: "var(--font-display)" }}>{cat.label}</h3>
                      {cat.count > 0 ? (
                        ...
                        <span className="text-[10px] font-semibold bg-[#f0ede6] text-[#aaa] px-2 py-0.5 rounded-full shrink-0" style={{ fontFamily: "var(--font-mono)" }}>Bientôt</span>
                      )}
                    </div>
        ...
        <section className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-end justify-between mb-10">
            <div>
              ...
              <h2 className="text-[28px] sm:text-[34px] font-semibold tracking-tight text-[#111218]" style={{ fontFamily: "var(--font-display)" }}>Fraîchement publiés</h2>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {articles[0] && (
              <Link href={`/${articles[0].category}/${articles[0].slug}`} className="group col-span-1 bg-white border border-[#e3e0d8] rounded-2xl p-6 card-hover block">
                ...
                <h3 className="text-[20px] font-semibold leading-snug tracking-tight text-[#111218] mt-3 mb-3 group-hover:text-[#059669] transition-colors" style={{ fontFamily: "var(--font-display)" }}>{articles[0].title}</h3>
                <p className="text-[14px] text-[#6b7280] line-clamp-3 leading-relaxed mb-4">{articles[0].description}</p>
                <div className="flex justify-between text-[12px] text-[#aaa] border-t border-[#f0ede6] pt-4">
            ...
            <div className="flex flex-col gap-5">
              {articles.slice(1, 3).map((a) => (
                <Link key={a.slug} href={`/${a.category}/${a.slug}`} className="group bg-white border border-[#e3e0d8] rounded-2xl p-5 card-hover block">
                  ...
                  <h3 className="text-[15px] font-semibold leading-snug text-[#111218] mt-2 mb-1.5 group-hover:text-[#059669] transition-colors line-clamp-2" style={{ fontFamily: "var(--font-display)" }}>{a.title}</h3>
                  <p className="text-[13px] text-[#6b7280] line-clamp-2">{a.description}</p>
                </Link>
              ))}
            </div>
          </div>
      ...
      {/* FEATURED HOSTINGS */}
      {featured.length > 0 && (
        <section className="border-t border-[#e3e0d8] py-16">
          <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-end justify-between mb-10">
              <div>
                <p className="section-label mb-2">Comparatif vedette</p>
                <h2 className="text-[28px] sm:text-[34px] font-semibold tracking-tight text-[#111218]" style={{ fontFamily: "var(--font-display)" }}>Meilleurs hébergeurs mutualisés</h2>
              </div>
              <Link href="/comparatifs/meilleur-hebergeur-web" className="hidden sm:flex items-center gap-1 text-[13px] text-[#6b7280] hover:text-[#059669] transition-colors font-medium">Comparatif complet →</Link>
            </div>
      ...
      {/* TRUST BAR */}
      <section className="border-y border-[#e3e0d8] py-8 overflow-hidden bg-white">
        ...
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <h2 className="text-[28px] sm:text-[34px] font-semibold tracking-tight text-[#111218] mb-4" style={{ fontFamily: "var(--font-display)" }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={s.label} className={`rounded-2xl border ${s.color} p-5`}>
                  <div className="text-[32px] font-bold text-[#111218] leading-none mb-0.5" style={{ fontFamily: "var(--font-display)" }}>
                    {s.val}{s.unit && <span className="text-[18px] text-[#6b7280] ml-0.5">{s.unit}</span>}
                  </div>
                  <p className="text-[12px] text-[#6b7280] leading-snug mt-1">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
